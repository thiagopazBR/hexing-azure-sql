...
import * as data_validation from './functions/data_validation'
...
;(async () => {
  ...
  while (i--) {
    ...
    row = data_validation['data_validation_commissioning_report'](row)
    ...
    const query: string = prepare_query(row)
    await mssql.select(query)
    console.log(row['Device ID'])
  }
})()
